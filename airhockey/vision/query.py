...
from airhockey.vision.transform import Perspective, Transform
...
class PositionQuery(Query):
    ...
    def execute(self, hsv, translator: Transform, debug_window):
...
class QueryContext(object):
    ...
    def __enter__(self):
        self.frame, self.frame_hsv = self.frame_reader.read()

        self.queries = []
        if self.debug_window is not None:
            self.debug_window.set_frame(self.frame, self.frame_hsv)
        self.can_execute = True
        self.perspective.update_transform(self.frame_hsv)
        return self

    def __exit__(self, exc_type, exc_val, exc_tb):
        self.can_execute = False
        if self.debug_window is not None:
            with self.debug_window:
                for query in self.queries:
                    query.draw(self.debug_window)
                self.debug_window.draw(self.frame_reader)
                self.perspective.draw(self.debug_window)
        self.frame = None
        self.frame_hsv = None
    ...
    def query(self, query: Query) -> Any:
        ...
        return query.execute(
            self.frame_hsv, self.perspective, self.debug_window)
